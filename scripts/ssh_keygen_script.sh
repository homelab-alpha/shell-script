...
#
# Note:
# - If the script encounters any errors during key generation or conversion, it will exit with a non-zero status.
# - The script ensures that duplicate key filenames are not overwritten to prevent accidental data loss.
# - Verbose mode provides additional information about the key pair generation and conversion process, including the contents of the generated and converted public keys.
